...
The `/readings` page and home-page summary load today's Mass readings from [Universalis](https://universalis.com/n-web.htm) using its [documented JSONP integration](https://universalis.com/n-jsonp-technical.htm). The request selects the `africa.kenya` calendar and the current date in `Africa/Nairobi`. The site renders the publisher's source and full copyright notice, with a direct link to the dated Universalis page if the feed fails. It does not scrape pages or store reading texts in JSON.
